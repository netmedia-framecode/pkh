    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $to       = $data['email'];
          $subject  = "Account Verification - PKH";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di PKH.</p>
...
if (isset($_SESSION["project_pkh"]["users"])) {
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["project_pkh"]["name_page"] = "' . $data['title'] . '";
  ...
  function bantuan($conn, $data, $action)
  {
    if ($action == "insert") {
      $sql = "INSERT INTO bantuan(jenis_bantuan,nama_bantuan,bulan_bantuan) VALUES('$data[jenis_bantuan]','$data[nama_bantuan]','$data[bulan_bantuan]')";
    }

    if ($action == "update") {
      $sql = "UPDATE bantuan SET jenis_bantuan='$data[jenis_bantuan]', nama_bantuan='$data[nama_bantuan]', bulan_bantuan='$data[bulan_bantuan]' WHERE id_bantuan='$data[id_bantuan]'";
    }

    if ($action == "delete") {
      $sql = "DELETE FROM bantuan WHERE id_bantuan='$data[id_bantuan]'";
    }
  ...
  function pendamping($conn, $data, $action)
  {
    if ($action == "insert") {
      $check = "SELECT * FROM pendamping WHERE kode_pendamping='$data[kode_pendamping]'";
      $check = mysqli_query($conn, $check);
      if (mysqli_num_rows($check) > 0) {
        $message = "Maaf, kode pendamping yang anda masukan sudah ada.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "INSERT INTO pendamping(kode_pendamping,nama_pendamping) VALUES('$data[kode_pendamping]','$data[nama_pendamping]')";
    }

    if ($action == "update") {
      if ($data['kode_pendamping'] != $data['kode_pendampingOld']) {
        $check = "SELECT * FROM pendamping WHERE kode_pendamping='$data[kode_pendamping]'";
        $check = mysqli_query($conn, $check);
        if (mysqli_num_rows($check) > 0) {
          $message = "Maaf, kode pendamping yang anda masukan sudah ada.";
      ...
      $sql = "UPDATE pendamping SET kode_pendamping='$data[kode_pendamping]', nama_pendamping='$data[nama_pendamping]' WHERE id_pendamping='$data[id_pendamping]'";
    }

    if ($action == "delete") {
      $sql = "DELETE FROM pendamping WHERE id_pendamping='$data[id_pendamping]'";
    }
  ...
  function warga($conn, $data, $action)
  {
    if ($action == "insert") {
      $check = "SELECT * FROM warga WHERE nama='$data[nama]'";
      $check = mysqli_query($conn, $check);
      if (mysqli_num_rows($check) > 0) {
        $message = "Maaf, nama warga yang anda masukan sudah ada.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
      $sql = "INSERT INTO warga(id_pendamping,no_kk,nama,tempat_lahir,tgl_lahir,id_pendidikan,pekerjaan,penghasilan,no_hp,no_rek,kd_pos,desa_kel,kec,kab_kota,prov,jumlah_anak,jumlah_sd,jumlah_smp,jumlah_sma,jumlah_balita,jumlah_bumil,jumlah_lansia,jumlah_disabilitas,nama_desa) VALUES('$data[id_pendamping]','$data[no_kk]','$data[nama]','$data[tempat_lahir]','$data[tgl_lahir]','$data[id_pendidikan]','$data[pekerjaan]','$data[penghasilan]','$data[no_hp]','$data[no_rek]','$data[kd_pos]','$data[desa_kel]','$data[kec]','$data[kab_kota]','$data[prov]','$data[jumlah_anak]','$data[jumlah_sd]','$data[jumlah_smp]','$data[jumlah_sma]','$data[jumlah_balita]','$data[jumlah_bumil]','$data[jumlah_lansia]','$data[jumlah_disabilitas]','$data[nama_desa]')";
    }

    if ($action == "update") {
      if ($data['nama'] != $data['namaOld']) {
        $check = "SELECT * FROM warga WHERE nama='$data[nama]'";
        $check = mysqli_query($conn, $check);
        if (mysqli_num_rows($check) > 0) {
          $message = "Maaf, nama warga yang anda masukan sudah ada.";
      ...
      $sql = "UPDATE warga SET id_pendamping='$data[id_pendamping]', no_kk='$data[no_kk]', nama='$data[nama]', tempat_lahir='$data[tempat_lahir]', tgl_lahir='$data[tgl_lahir]', id_pendidikan='$data[id_pendidikan]', pekerjaan='$data[pekerjaan]', penghasilan='$data[penghasilan]', no_hp='$data[no_hp]', no_rek='$data[no_rek]', kd_pos='$data[kd_pos]', desa_kel='$data[desa_kel]', kec='$data[kec]', kab_kota='$data[kab_kota]', prov='$data[prov]', jumlah_anak='$data[jumlah_anak]', jumlah_sd='$data[jumlah_sd]', jumlah_smp='$data[jumlah_smp]', jumlah_sma='$data[jumlah_sma]', jumlah_balita='$data[jumlah_balita]', jumlah_bumil='$data[jumlah_bumil]', jumlah_lansia='$data[jumlah_lansia]', jumlah_disabilitas='$data[jumlah_disabilitas]', nama_desa='$data[nama_desa]' WHERE id_warga='$data[id_warga]'";
    }

    if ($action == "delete") {
      $sql = "DELETE FROM warga WHERE id_warga='$data[id_warga]'";
    }
  ...
  function berita($conn, $data, $action, $deskripsi)
  {
    if ($action == "insert") {
      $nama_berita = ucwords($data['nama_berita']);
      $sql = "INSERT INTO berita(nama_berita,deskripsi,author) VALUES('$nama_berita','$deskripsi','$data[author]')";
    }

    if ($action == "update") {
      $nama_berita = ucwords($data['nama_berita']);
      $sql = "UPDATE berita SET nama_berita='$nama_berita', deskripsi='$deskripsi', author='$data[author]' WHERE id_berita='$data[id_berita]'";
    }
  ...
  function visi($conn, $data, $action, $deskripsi)
  {
    if ($action == "update") {
      $sql = "UPDATE visi SET deskripsi='$deskripsi'";
    }
  ...
  function misi($conn, $data, $action, $deskripsi)
  {
    if ($action == "update") {
      $sql = "UPDATE misi SET deskripsi='$deskripsi'";
    }
  ...
  {
    if ($action == "update") {
      $sql = "UPDATE sejarah SET deskripsi='$deskripsi'";
    }
